...
@Service
@Slf4j
public class AssessmentsRegistryDataMigrationWFClient implements DataMigrationWfClient {

  private final WorkflowService workflowService;
  private final WorkflowClientService workflowClientService;

  public AssessmentsRegistryDataMigrationWFClient(WorkflowService workflowService, WorkflowClientService workflowClientService) {
    this.workflowService = workflowService;
    this.workflowClientService = workflowClientService;
  }

  public WorkflowCreatedDTO migrate(Long uploadId) {
    log.info("Starting assessments registry data migration having id {}", uploadId);
    String taskQueue = WfConstants.TASK_QUEUE_MIGRATION;
    String workflowId = WfUtilities.generateWorkflowId(uploadId, AssessmentsRegistryDataMigrationWF.class);

    AssessmentsRegistryDataMigrationWF workflow = workflowService.buildWorkflowStub(
      AssessmentsRegistryDataMigrationWF.class,
      taskQueue,
      workflowId);
