...
class DebtPositionsDataMigrationWFClientTest {
  ...
  @Mock
  private DebtPositionsDataMigrationWF wfMock;

  private DebtPositionsDataMigrationWFClient client;

  @BeforeEach
  void setUp() {
    client = new DebtPositionsDataMigrationWFClient(workflowServiceMock, workflowClientServiceMock);
  }
  ...
  void whenMigrateThenInvokeWfClient() {
    ...
    WorkflowCreatedDTO expectedResult = new WorkflowCreatedDTO("DebtPositionsDataMigrationWF-123", "RUNID");

    Mockito.when(workflowServiceMock.buildWorkflowStub(DebtPositionsDataMigrationWF.class, taskQueue, expectedResult.getWorkflowId()))
      .thenReturn(wfMock);
