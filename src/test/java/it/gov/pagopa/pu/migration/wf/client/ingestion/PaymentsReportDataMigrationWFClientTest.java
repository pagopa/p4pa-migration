...
class PaymentsReportDataMigrationWFClientTest {
  ...
  @Mock
  private PaymentsReportingDataMigrationWF wfMock;

  private PaymentsReportingDataMigrationWFClient client;

  @BeforeEach
  void setUp() {
    client = new PaymentsReportingDataMigrationWFClient(workflowServiceMock, workflowClientServiceMock);
  }
  ...
  void whenMigrateThenInvokeWfClient() {
    ...
    WorkflowCreatedDTO expectedResult = new WorkflowCreatedDTO("PaymentsReportingDataMigrationWF-123", "RUNID");

    Mockito.when(workflowServiceMock.buildWorkflowStub(PaymentsReportingDataMigrationWF.class, taskQueue, expectedResult.getWorkflowId()))
      .thenReturn(wfMock);
