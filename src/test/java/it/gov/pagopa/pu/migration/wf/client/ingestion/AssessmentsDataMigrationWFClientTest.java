...
class AssessmentsDataMigrationWFClientTest {
  ...
  @Mock
  private AssessmentsDataMigrationWF wfMock;

  private AssessmentsDataMigrationWFClient client;

  @BeforeEach
  void setUp() {
    client = new AssessmentsDataMigrationWFClient(workflowServiceMock, workflowClientServiceMock);
  }
  ...
  void whenMigrateThenInvokeWfClient() {
    ...
    WorkflowCreatedDTO expectedResult = new WorkflowCreatedDTO("AssessmentsDataMigrationWF-123", "RUNID");

    Mockito.when(workflowServiceMock.buildWorkflowStub(AssessmentsDataMigrationWF.class, taskQueue, expectedResult.getWorkflowId()))
      .thenReturn(wfMock);
