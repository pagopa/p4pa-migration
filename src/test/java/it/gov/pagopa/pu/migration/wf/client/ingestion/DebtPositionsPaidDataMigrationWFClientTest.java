...
class DebtPositionsPaidDataMigrationWFClientTest {
  ...
  @Mock
  private DebtPositionsPaidDataMigrationWF wfMock;

  private DebtPositionsPaidDataMigrationWFClient client;

  @BeforeEach
  void setUp() {
    client = new DebtPositionsPaidDataMigrationWFClient(workflowServiceMock, workflowClientServiceMock);
  }
  ...
  void whenMigrateThenInvokeWfClient() {
    ...
    WorkflowCreatedDTO expectedResult = new WorkflowCreatedDTO("DebtPositionsPaidDataMigrationWF-123", "RUNID");

    Mockito.when(workflowServiceMock.buildWorkflowStub(DebtPositionsPaidDataMigrationWF.class, taskQueue, expectedResult.getWorkflowId()))
      .thenReturn(wfMock);
