...
class OrgSilServiceDataMigrationWFClientTest {
  ...
  @Mock
  private OrgSilServiceDataMigrationWF wfMock;

  private OrgSilServiceDataMigrationWFClient client;

  @BeforeEach
  void setUp() {
    client = new OrgSilServiceDataMigrationWFClient(workflowServiceMock, workflowClientServiceMock);
  }
  ...
  void whenMigrateThenInvokeWfClient() {
    ...
    WorkflowCreatedDTO expectedResult = new WorkflowCreatedDTO("OrgSilServiceDataMigrationWF-123", "RUNID");

    Mockito.when(workflowServiceMock.buildWorkflowStub(OrgSilServiceDataMigrationWF.class, taskQueue, expectedResult.getWorkflowId()))
      .thenReturn(wfMock);
