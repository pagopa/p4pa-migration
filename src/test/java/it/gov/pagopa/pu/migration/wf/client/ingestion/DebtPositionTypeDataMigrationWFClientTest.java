...
class DebtPositionTypeDataMigrationWFClientTest {
  ...
  @Mock
  private DebtPositionTypeDataMigrationWF wfMock;

  private DebtPositionTypeDataMigrationWFClient client;

  @BeforeEach
  void setUp() {
    client = new DebtPositionTypeDataMigrationWFClient(workflowServiceMock, workflowClientServiceMock);
  }
  ...
  void whenMigrateThenInvokeWfClient() {
    ...
    WorkflowCreatedDTO expectedResult = new WorkflowCreatedDTO("DebtPositionTypeDataMigrationWF-123", "RUNID");

    Mockito.when(workflowServiceMock.buildWorkflowStub(DebtPositionTypeDataMigrationWF.class, taskQueue, expectedResult.getWorkflowId()))
      .thenReturn(wfMock);
