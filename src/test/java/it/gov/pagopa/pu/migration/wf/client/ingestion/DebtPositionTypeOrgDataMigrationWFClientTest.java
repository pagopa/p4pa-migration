...
import it.gov.pagopa.pu.migration.wf.wf.ingestion.debtpositiontypeorg.DebtPositionTypeOrgDataMigrationWF;
...
class DebtPositionTypeOrgDataMigrationWFClientTest {
  ...
  @Mock
  private DebtPositionTypeOrgDataMigrationWF wfMock;

  private DebtPositionTypeOrgDataMigrationWFClient client;

  @BeforeEach
  void setUp() {
    client = new DebtPositionTypeOrgDataMigrationWFClient(workflowServiceMock, workflowClientServiceMock);
  }
  ...
  void whenMigrateThenInvokeWfClient() {
    ...
    WorkflowCreatedDTO expectedResult = new WorkflowCreatedDTO("DebtPositionTypeOrgDataMigrationWF-123", "RUNID");

    Mockito.when(workflowServiceMock.buildWorkflowStub(DebtPositionTypeOrgDataMigrationWF.class, taskQueue, expectedResult.getWorkflowId()))
      .thenReturn(wfMock);
